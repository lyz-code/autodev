...
from autodev.entrypoints.cli import cli
from autodev.version import __version__
...
def fixture_runner(config: Config) -> CliRunner:
    ...
    return CliRunner(mix_stderr=False, env={"AUTODEV_CONFIG_PATH": config.config_path})
...
def test_version(runner: CliRunner) -> None:
    ...
    assert re.match(
        fr" *autodev version: {__version__}\n" r" *python version: .*\n *platform: .*",
        result.stdout,
    )
